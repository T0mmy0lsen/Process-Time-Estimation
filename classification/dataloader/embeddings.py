...
class GloVe:
    ...
    def _load(self):
        ...
        with open(os.path.join(config.GLOVE_DIR, 'glove.6B.' + str(self.EMBEDDING_DIM) + 'd.txt'),
    ...
    def _init_embedding_matrix(self, word_index_dict, oov_words_file='data/OOV-Words.txt'):
        self.embedding_matrix = np.zeros(
            (len(word_index_dict) + 2, self.EMBEDDING_DIM))  # +1 for the 0 word index from paddings.
        not_found_words = 0
        missing_word_index = []
...
# Test
# glove = Word2Vec(50)
# initial_embeddings = glove.get_embedding({'good': 2, 'movie': 3, 'sad': 1})