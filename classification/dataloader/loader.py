...
from classification.dataloader.ihlp import IHLP
from classification.preprocessing import utils
from objs.Request import Requests, Request
...
class Loader:
        ...
        for sentiment in ('pos', 'neg'):
            ...
            for review_file in os.listdir(path):
                with open(os.path.join(path, review_file), 'r', encoding='utf-8') as input_file:
                    ...
                    df = df.append([[utils.strip_html_tags(review), labels[sentiment]]], ignore_index=True)
    ...
    @staticmethod
    def load_imdb_unsup(datafile=None):
        df = pd.DataFrame()
    ...
    def load_reuters(self, test_or_train='train'):
        # List of categories
        data = []
    ...
    def load_processed_20newsgrp_data(self, test_or_train='train'):
